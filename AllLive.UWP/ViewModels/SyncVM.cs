...
namespace AllLive.UWP.ViewModels
{
    public class SyncVM : BaseViewModel
    {
        ...
        private void ReceiveFavorite(bool overlay, string content)
        {
            ...
            var items = JsonSerializer.Deserialize<List<FavoriteJsonItem>>(content);
        ...
        private void ReceiveHistory(bool overlay, string content)
        {
            ...
            var items = JsonSerializer.Deserialize<List<HistoryJsonItem>>(content);
            foreach (var item in items)
            {
        ...
        private void ReceiveShieldWord(bool overlay, string content)
        {
            ...
            SettingHelper.SetValue(SettingHelper.LiveDanmaku.SHIELD_WORD, JsonSerializer.Serialize(currentWords));
            ShowMessage("已同步屏蔽词");
        ...
        public async void SendShieldWord()
        {
            ...
            var json = JsonSerializer.Serialize(currentWords);
